...
function insRow(table_id, block, rowIndex) {
    ...
    let x = document.getElementById(table_id).insertRow(rowIndex);
    //每一列的数据
    let c0 = x.insertCell(0);
    let c1 = x.insertCell(1);
    let c2 = x.insertCell(2);
    let c3 = x.insertCell(3);
    console.log(web3.eth.getBlock(block));
    ...
    let blockHash = new BigNumber(web3.eth.getBlock(block).hash).toString(16);
    let temp = JSON.stringify(blockHash);
