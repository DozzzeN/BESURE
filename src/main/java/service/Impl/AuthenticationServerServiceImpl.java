...
public class AuthenticationServerServiceImpl implements AuthenticationServerService {
    ...
    public int createUser(String username, String password) {
        int ins = userMapper.insUser(username, password, "creator");
        return ins > 0 ? ins : -1;
    }
    ...
    public Map<String, Object> receiveC1C2FromUser_sendC3C4ToUser(Map<String, Object> map) {
        ...
        //计算k=s*C_1
        Element k = C_1.duplicate().mulZn(SysParamServiceImpl.skH).getImmutable();
        //解密C_2
        byte[] k_hash = CryptoUtil.getHash("SHA-256", k);//将k哈希，使其长度适应aes算法的密钥长度
        byte[] decry_C_2 = CryptoUtil.AESDecrypt(k_hash, C_2);
        //获取信息
        ...
        //虚拟身份PID
        byte[] PID = CryptoUtil.AESEncrypt(k_hash, mergeData1);
        ...
        byte[] C_4 = CryptoUtil.AESEncrypt(sk_hash, mergeData2);
    ...
    public byte[] sendSignatureToUser(User user, byte[] encryptedState) {
        ...
        Element signature = state_PID_hashValue.duplicate().mulZn(SysParamServiceImpl.skH.duplicate()).getImmutable();
        logger.warn("第二阶段：AS给用户发送签名：" + signature.toString());
        return CryptoUtil.AESEncrypt(CryptoUtil.getHash("SHA-256", AuthenticationServer.getSk_as()), signature.toBytes());
    }
    ...
    public byte[] sendEditSignatureToUser(User user, byte[] encryptedState, byte[] encryptedBlockHash, byte[] encryptedSignature) {
        ...
        byte[] lastBlockHash = CryptoUtil.AESDecrypt(CryptoUtil.getHash("SHA-256", AuthenticationServer.getSk_as()), encryptedBlockHash);
        ...
        Element thisSignature = Theta_St.duplicate().mulZn(SysParamServiceImpl.skH).getImmutable();
        logger.warn("第二阶段：AS将当前签名发送给用户");
        return CryptoUtil.AESEncrypt(CryptoUtil.getHash("SHA-256", AuthenticationServer.getSk_as()), thisSignature.toBytes());
    }
}
